...
def test_record_email_persists_introspection_when_present(fake_db):
    """AUTO-33: the stage-5 extraction-diagnostic sub-document lands under
...
def test_record_email_omits_introspection_when_none(fake_db):
    """Emails that exit before stage 5 carry no introspection — the key is
    absent rather than a null, keeping non-stage-5 docs lean."""
    triage_store.record_email(
        "run-1",
